...
export class UserTableComponent {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'mobile', headerName: 'Contact No.', flex: 2 },
    {
      headerName: 'Actions',
  ...
  updateItem(item: any): void {
    ...
    this.router.navigateByUrl(`/authentication/signup/${item._id}`)
  }

  deleteItem(item: any): void {
    console.log('Delete item:', item._id)
    this._userService.deleteUser(item._id).subscribe(
      (res: any) => {
  ...
  viewPage(item: any) {
    ...
    this.router.navigateByUrl(`pages/viewUser/${item._id}`)
  }
}
